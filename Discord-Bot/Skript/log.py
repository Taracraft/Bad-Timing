...
DISCORD_BOT_TOKEN = "OTEwMTIxNjc1MjQ3NDYwMzky.YZOO3A.HqoFzMTM2c9EPO8FUniQDFu1NfY"


WATCHING_STATUS = "Überprüft Streams und Audit"

MUTED_ROLE_NAME = "Muted"
MOD_LOG_CHANNEL_NAME = "adminlog"
...
@client.event
async def on_ready():
    await client.change_presence(activity = discord.Activity(name = WATCHING_STATUS, type = discord.ActivityType.watching))
...
async def on_member_update(before, after):
    if before.roles == after.roles:
        return
    muted_role = discord.utils.get(after.guild.roles, name = MUTED_ROLE_NAME)
    if not muted_role:
        return
...
async def post_modlog(guild, type, user, target, reason):
    ...
    async for s in mod_log_channel.history(limit = 100):
        ...
        break
    e = discord.Embed(color = MODLOG_COLORS[type], timestamp = datetime.datetime.utcnow())
...
async def on_message(msg):
    if msg.type != discord.MessageType.default or not msg.guild or msg.author.bot or not msg.content:
        return
    if msg.content.startswith(".reason ") and msg.channel.name == MOD_LOG_CHANNEL_NAME:
